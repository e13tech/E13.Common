...
namespace Microsoft.AspNetCore.Authentication
{
    public static class AadIssuerValidator
    {
        ...
        public static string ValidateAadIssuer(string issuer, SecurityToken securityToken, TokenValidationParameters validationParameters)
        {
            ...
            allValidIssuers.AddRange(allValidIssuers.Select(i => i.Replace("https://login.microsoftonline.com", "https://sts.windows.net", StringComparison.InvariantCultureIgnoreCase)).ToArray());

            // Consider tokens provided both by v1.0 and v2.0 issuers
            allValidIssuers.AddRange(allValidIssuers.Select(i => i.Replace("/v2.0", "/", System.StringComparison.InvariantCultureIgnoreCase)).ToArray());
        ...
        {
            return i.Replace("{tenantid}", tenantId, System.StringComparison.InvariantCultureIgnoreCase);
        }
    }
}
